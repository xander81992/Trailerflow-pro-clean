...
function useTrailerData() {
  ...
  return { data, toast, addWarehouse, addDoor, addTrailer, createRequest, approveAndAssign, updateTaskStatus, createInvite, resetDemo };
}
...
function Warehouses({ user, store }) {
  const [form, setForm] = useState({ name: '', code: '', address: '', doors: 0 });
  const save = () => {
    try { store.addWarehouse(form, user); setForm({ name: '', code: '', address: '', doors: 0 }); } catch (e) { alert(e.message); }
  };
  return <div className="grid-2">
    <div className="card"><h2>Create Warehouse Location</h2><p className="card-sub">Admin creates real WHSE locations and initial doors.</p><div className="form-grid">
      ...
      <Field label="Initial Doors"><input type="number" min="0" value={form.doors} onChange={(e) => setForm({ ...form, doors: e.target.value })} /></Field>
    </div><div className="form-actions"><button className="btn btn-primary" onClick={save}>Create Warehouse</button></div></div>
    <div className="card"><h2>Warehouse List</h2><div className="table-wrap"><table><thead><tr><th>Name</th><th>Code</th><th>Doors</th><th>Occupied</th></tr></thead><tbody>{store.data.warehouses.map((w) => { const doors = store.data.doors.filter((d) => d.warehouseId === w.id); return <tr key={w.id}><td><strong>{w.name}</strong><br /><small>{w.address}</small></td><td>{w.code}</td><td>{doors.length}</td><td>{doors.filter((d) => d.trailerId).length}</td></tr>; })}</tbody></table></div></div>
  </div>;
}

function Doors({ user, store }) {
  const [warehouseId, setWarehouseId] = useState(store.data.warehouses[0]?.id || '');
  const [code, setCode] = useState('');
  const save = () => { try { store.addDoor(warehouseId, code, user); setCode(''); } catch (e) { alert(e.message); } };
  return <div className="section-stack">
    <div className="card"><h2>Add Door</h2><div className="form-grid"><Field label="Warehouse"><select value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>{store.data.warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></Field><Field label="Door Code"><input value={code} onChange={(e) => setCode(e.target.value)} placeholder="A9" /></Field></div><div className="form-actions"><button className="btn btn-primary" onClick={save}>Add Door</button></div></div>
    <div className="card"><h2>Door Occupancy</h2><p className="card-sub">Business rule: one door can only have one trailer.</p><YardMap data={store.data} /></div>
  </div>;
}

function Trailers({ user, store, search }) {
  const data = store.data;
  const [form, setForm] = useState({ number: '', plate: '', companyId: 'rnf', status: 'Empty', warehouseId: '', doorId: '', notes: '' });
  const availableDoors = data.doors.filter((d) => !d.trailerId && (!form.warehouseId || d.warehouseId === form.warehouseId));
  const save = () => { try { store.addTrailer(form, user); setForm({ number: '', plate: '', companyId: 'rnf', status: 'Empty', warehouseId: '', doorId: '', notes: '' }); } catch (e) { alert(e.message); } };
  ...
  return <div className="section-stack">
    <div className="card"><h2>Create Trailer Number</h2><div className="form-grid">
      ...
      <Field label="Warehouse"><select value={form.warehouseId} onChange={(e) => setForm({ ...form, warehouseId: e.target.value, doorId: '' })}><option value="">No location</option>{data.warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></Field>
